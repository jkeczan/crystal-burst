...
6. Search + Replace all instances of 'crystal-burst' with your project name
7. Search + Replace 'recklanlabs' with your company name.  This is the NX project scope that allows you to import libraries using @recklanlabs

That's it for the renaming!  Now to test...

```npm install```

```npx nx run crystal-burst:serve``` (where crystal-burst is the name you replaced with above)
...
Libraries are shareable across libraries and applications. They can be imported from `@crystal-burst/mylib`.

## Development server

Run `npx nx run crystal-burst:serve` for a dev server. Navigate to http://localhost:4200/. The app will automatically reload if you change any of the source files.
